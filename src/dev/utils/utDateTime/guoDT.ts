...
const getDayjs = (dayInput?: string | number | Date): Dayjs => {
    ...
    if (typeof dayInput === 'string') {
        ...
        return dayjs.tz(dayInput, DEFAULT_TIMEZONE)
    }
...
const getFormattedDayjs = (dayjsObj: Dayjs, formatString = "YYYY年M月D日 HH:mm:ss") => {
    ...
    if (!dayjsObj && !formatString) {
        return dayjs().tz(DEFAULT_TIMEZONE).format("YYYY年M月D日 HH:mm:ss");
    }
    if (dayjsObj) {
        return dayjsObj.tz(DEFAULT_TIMEZONE).format(formatString)
    }
    return dayjs().tz(DEFAULT_TIMEZONE).format(formatString)
}
...
const formatToApiDateTime = (input: Date | string | number): string => {
    return getDayjs(input).format(API_DATE_TIME_FORMAT)
}

const formatToDisplayWithZone = (input: Date | string | number, format = 'YYYY-MM-DD HH:mm:ss [UTC]Z') => {
    const parsed = getDayjs(input)
