...
const FadeSwitchPicOne = (props: {
    ...
    const { opacityAnim } = useMemo(() => {
        if (isCrossDissolve) {
            ...
            const timeline = [] as { durationSeconds: number; toValue: number }[];
            if (fadeOutStart > 0) timeline.push({ durationSeconds: fadeOutStart, toValue: 1 }); // 保持可见直到退场开始
            if (fadeDuration > 0) timeline.push({ durationSeconds: fadeDuration, toValue: 0 }); // 退场（淡出）
            if (waitEnd > 0) timeline.push({ durationSeconds: waitEnd, toValue: 0 }); // 屏外等待到周期结束
            ...
            const timeline = [] as { durationSeconds: number; toValue: number }[];
            if (fadeInStart > 0) timeline.push({ durationSeconds: fadeInStart, toValue: 0 });
            if (fadeDuration > 0) timeline.push({ durationSeconds: fadeDuration, toValue: 1 });
            timeline.push({ durationSeconds: stayDuration, toValue: 1 });
            if (fadeDuration > 0) timeline.push({ durationSeconds: fadeDuration, toValue: 0 });
            if (waitEnd > 0) timeline.push({ durationSeconds: waitEnd, toValue: 0 });
