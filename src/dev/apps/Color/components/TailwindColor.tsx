...
type FormatType = 'hex' | 'hashHex' | 'rgb' | 'hsl' | 'oklch';

export const TailwindColor: React.FC<TailwindColorProps> = ({ onColorChange }) => {
  ...
  const copyToClipboard = async (color: string) => {
    try {
      ...
      // 如果是HEX格式，同时更新主颜色选择器
      if ((format === 'hex' || format === 'hashHex') && color.startsWith('#')) {
  ...
  const formatColor = (hex: string, formatType: FormatType): string => {
    switch (formatType) {
      case 'hex':
        return hex.replace('#', '');
      case 'hashHex':
      ...
      case 'oklch': {
        // 使用现有的OKLCH转换函数
        const { rgbaToOklch, formatOklch } = require('../utils/color');
        const r = parseInt(hex.slice(1, 3), 16);
        const g = parseInt(hex.slice(3, 5), 16);
        const b = parseInt(hex.slice(5, 7), 16);
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-center gap-3">
        <span className="text-sm font-medium text-foreground">颜色格式:</span>
        <div className="inline-flex items-center gap-1 rounded-lg border bg-background p-1 shadow-sm">
          {(['hex', 'hashHex', 'rgb', 'hsl', 'oklch'] as FormatType[]).map((formatType) => {
            const labels = {
              'hex': 'HEX',
              'hashHex': '#HEX',
              'rgb': 'RGB',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 max-w-6xl mx-auto">
        {Object.entries(tailwindColors).map(([colorName, colorShades]) => (
          <div key={colorName} className="bg-card rounded-xl border p-6 shadow-sm">
            {/* 卡片标题 */}
      ...
      <div className="text-center text-sm text-muted-foreground bg-muted/50 rounded-lg p-3">
        点击任意颜色方块复制到剪贴板。HEX格式会同时更新主颜色选择器。
      </div>
    </div>
  );
};