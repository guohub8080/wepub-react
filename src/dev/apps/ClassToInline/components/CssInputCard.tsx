...
import { Upload, Trash2, Maximize2, Type, Code, Copy, Scissors } from "lucide-react";
...
interface CssInputCardProps {
  onOpenFullscreen: (title: string, content: string, language: 'xml' | 'jsx') => void;
}

const CssInputCard: React.FC<CssInputCardProps> = ({ onOpenFullscreen }) => {
  const { cssInput, setCssInput, setResultOutput } = useClassToInlineStore();
  ...
  // 格式化代码
  useEffect(() => {
    if (highlight && cssInput) {
      formatXmlWithAttributeBreaks(cssInput).then(setFormattedCode);
    } else if (!cssInput) {
      setFormattedCode('');
    }
  }, [highlight, cssInput]);
  ...
  // 移除 @font-face
  const handleRemoveFontFace = () => {
    if (!cssInput) return;
    // 移除所有 @font-face 声明（包括完整的 @font-face 块）
    const cleaned = cssInput.replace(/@font-face\s*{[^}]*}/gs, '');
    setCssInput(cleaned);
  };
      ...
      <CardHeader className="gap-0 border-b h-14 flex items-center">
        <CardTitle className="text-base flex items-center justify-between w-full">
          ...
          <div className="flex items-center gap-1">
            {/* 移除 @font-face 按钮 */}
            {cssInput && cssInput.includes('@font-face') && (
              <Button
                variant="ghost"
                size="sm"
                onClick={handleRemoveFontFace}
            ...
            {cssInput && (
              <Button
                variant="ghost"
                size="sm"
                onClick={() => onOpenFullscreen('CSS 输入', cssInput, 'jsx')}
                className="h-6 w-6 p-0"
                title="全屏查看"
              >
        ...
        <div className="w-full h-full overflow-auto">
          {highlight ? (
            <SyntaxHighlighter
              ...
              CodeTag="code"
            >
              {formattedCode || cssInput || '/* 粘贴CSS代码到这里 */'}
            </SyntaxHighlighter>
          ) : (
            <textarea
              value={cssInput}
