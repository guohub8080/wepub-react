...
import placeholder1 from "@assets/pics/placeholders/300x500/3.jpg";
import placeholder2 from "@assets/pics/placeholders/300x300/4.jpg";
import { getDayjs } from "@utils/utDateTime/exDayjs";
import wechatColors from "@aieco/presets/color";

const articleMeta = {
    title: "AIECO红色主题模板",
    date: getDayjs('2024-12-02 00:00:00'),
    author: "guohub8080",
    tag: ["AIECO", "模板"],
}
...
const Article = () => {
    return (<AIECO>
        <HeadAIECO />
        <ArticleFrame theme="red" underTitle={underTitle}>
            <P>这是一段红色主题的测试文字。</P>
            <P>这是一段用于测试排版的文字，主要目的是验证 AIECO 模板红色主题在显示多段落内容时的效果。</P>
            <ImgFrame1
                url={placeholder1}
                description="这是一张测试图片，用于验证红色主题图片组件的显示效果。"
            />
            <P>继续测试红色主题。这里是一段中等长度的文字，用来观察不同长度的段落在一起时的视觉效果。</P>
            <P textIndent={0}>这是无首行缩进的文字。</P>
            <P textIndent={0} align="center">这是无首行缩进+正文居中的文字。</P>
            <P textIndent={0} align="center"><B color={wechatColors.titleRed}>这是红色加粗居中文字。</B></P>
            <P>这是一段更长的测试文字。在实际使用中，您可以根据需要在这里输入真正的文章内容。目前这些文字只是为了演示模板的排版功能而临时添加的占位符。红色主题适合用于党建、政务等重要类型的文章。</P>
            <ImgFrame1
                url={placeholder2}
