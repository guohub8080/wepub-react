...
import CoverOut from "@dev/pubComponents/SVG/C3_MultiDisplay/CoverOut";
import { 
    H1, 
    H2, 
    P, 
...
import { getEaseBezier, getPowerBezier, getLinearBezier } from "@/dev/pubUtils/getBezier/index.ts";
    ...
    return (
        <SectionEx className="multi-display-presets">
            ...
            <div style={{ padding: '0 16px' }}>
                <P>
                    <code>CoverOut</code> 组件实现图片的退出式切换，图片依次滑出，露出下层图片。
                </P>
                    ...
                    <table style={{ width: '100%', fontSize: '13px', borderCollapse: 'collapse' }}>
                        ...
                        <tbody>
                            ...
                            <tr style={{ borderBottom: '1px solid #eee' }}>
                                ...
                                <td style={{ padding: '6px', fontSize: '12px' }}>滑出方向（默认 B-往下退出）</td>
                            </tr>
                            <tr style={{ borderBottom: '1px solid #eee' }}>
                                <td style={{ padding: '6px', fontFamily: 'monospace' }}>pics[].coverOutDuration</td>
                                <td style={{ padding: '6px', fontSize: '12px', color: '#666' }}>number</td>
                                <td style={{ padding: '6px', fontSize: '12px' }}>滑出时长（秒，默认 0.5）</td>
                            </tr>
                            <tr style={{ borderBottom: '1px solid #eee' }}>
            ...
            <div style={{ padding: '0 16px' }}>
                <P>默认往下滑出，可通过 direction 参数设置方向（L/R/T/B）</P>
...
/>`} />
            ...
            <div style={{ padding: '0 16px' }}>
                ...
                <CodeBlock code={`<CoverOut
    pics={[
        { url: pic1, direction: "B", coverOutDuration: 0.8, stayDuration: 1.2, keySplines: getEaseBezier({ isIn: true, isOut: true }) },
        { url: pic2, direction: "L", coverOutDuration: 0.6, stayDuration: 1, keySplines: getPowerBezier({ power: 2, isIn: true, isOut: true }) },
        { url: pic3, direction: "T", coverOutDuration: 0.7, stayDuration: 1.5, keySplines: getEaseBezier({ isIn: false, isOut: true }) },
        // ... 更多配置
    ]}
/>`} />
            </div>
            ...
            <div style={{ padding: '0 16px' }}>
                ...
                <div style={{ 
                    ...
                    <CoverOut
                        pics={[
                            { url: pic1, direction: "B", coverOutDuration: 0.5, stayDuration: 0.8, keySplines: getLinearBezier() },
                            { url: pic2, direction: "L", coverOutDuration: 0.7, stayDuration: 1.1, keySplines: getEaseBezier({ isIn: true, isOut: false }) },
                            { url: pic3, direction: "T", coverOutDuration: 0.4, stayDuration: 0.9, keySplines: getPowerBezier({ power: 2, isIn: false, isOut: true }) },
                        ]}
                    />
                    {/* 右上 - 4张图，各自随机 */}
                    <CoverOut
                        pics={[
                            { url: pic4, direction: "R", coverOutDuration: 0.6, stayDuration: 1.2, keySplines: getEaseBezier({ isIn: true, isOut: true }) },
                            { url: pic5, direction: "B", coverOutDuration: 0.9, stayDuration: 0.7, keySplines: getLinearBezier() },
                            { url: pic6, direction: "L", coverOutDuration: 0.5, stayDuration: 1.4, keySplines: getPowerBezier({ power: 3, isIn: true, isOut: false }) },
                            { url: pic1, direction: "T", coverOutDuration: 0.8, stayDuration: 1.0, keySplines: getEaseBezier({ isIn: false, isOut: true }) },
                        ]}
                    />
                    {/* 左下 - 2张图，各自随机 */}
                    <CoverOut
                        pics={[
                            { url: pic2, direction: "R", coverOutDuration: 1.0, stayDuration: 0.6, keySplines: getPowerBezier({ power: 4, isIn: true, isOut: true }) },
                            { url: pic3, direction: "B", coverOutDuration: 0.4, stayDuration: 1.5, keySplines: getLinearBezier() },
                        ]}
                    />
                    {/* 右下 - 5张图，各自随机 */}
                    <CoverOut
                        pics={[
                            { url: pic4, direction: "T", coverOutDuration: 0.3, stayDuration: 1.3, keySplines: getEaseBezier({ isIn: true, isOut: false }) },
                            { url: pic5, direction: "L", coverOutDuration: 0.8, stayDuration: 0.9, keySplines: getPowerBezier({ power: 2, isIn: false, isOut: true }) },
                            { url: pic6, direction: "B", coverOutDuration: 0.6, stayDuration: 1.1, keySplines: getLinearBezier() },
                            { url: pic1, direction: "R", coverOutDuration: 0.9, stayDuration: 0.7, keySplines: getEaseBezier({ isIn: true, isOut: true }) },
                            { url: pic2, direction: "T", coverOutDuration: 0.5, stayDuration: 1.2, keySplines: getEaseBezier({ isIn: false, isOut: true }) },
                        ]}
                    />
                </div>
            ...
            <div style={{ padding: '0 16px', marginBottom: '20px' }}>
                <div style={{ background: '#e8f5e9', padding: '12px', borderRadius: '4px', border: '1px solid #4caf50' }}>
                    ...
                    <ul style={{ fontSize: '13px', margin: 0, paddingLeft: '20px', lineHeight: '1.8' }}>
                        <li><strong>方向选择：</strong>B（往下退出）最为常用，符合重力感</li>
                        <li><strong>时长设置：</strong>滑出时长 0.3-0.8 秒，停留时长根据内容调整</li>
                        <li><strong>缓动函数：</strong>退出动画建议使用 ease-in，更自然</li>
                        <li><strong>图片数量：</strong>建议 3-5 张，过多会影响节奏感</li>
                        <li><strong>图层顺序：</strong>第一张图在最上层，最后一张在最下层</li>
                    </ul>
                </div>
                ...
                <div style={{ background: '#fff3cd', padding: '12px', borderRadius: '4px', marginTop: '12px', border: '1px solid #ffc107' }}>
                    ...
                    <ul style={{ fontSize: '13px', margin: 0, paddingLeft: '20px', lineHeight: '1.8' }}>
                        <li>方向参数：T-往上退出, B-往下退出, L-往左退出, R-往右退出</li>
                        <li>所有图片初始叠加显示，第一张在最上层</li>
                        <li>图片按顺序依次滑出，露出下层图片</li>
                        <li>循环时所有图片会同时重置到初始位置</li>
                    </ul>
                </div>
            </div>
        </SectionEx>
