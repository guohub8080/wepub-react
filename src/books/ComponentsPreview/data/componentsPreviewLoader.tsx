/**
 * ComponentsPreview 组件预览动态加载器
 * 自动扫描 src/books/ComponentsPreview 文件夹下的所有 .tsx, .md, .mdx 文件
 * 按文件夹分组，每个文件夹对应一个分类
 *
 * 文件夹命名规则：[x]M
 * - [x] - 排序号，数字越小越靠上
 ...
 * - [x] - 排序号（数字，组内排序）
 * - slug - URL 路径（全英文，如：button, modal）
 * - 扩展名：.tsx（React组件）、.md（Markdown）、.mdx（MDX）
 *
 * 例如：文件夹 [01]基础组件/info.tsx 定义 slug: "basic"
 *       文件 [1]button.tsx => 路由为 /{slug}/basic/button
 *       文件 [2]modal.md => 路由为 /{slug}/basic/modal
...
import { componentsPreviewConfig } from './info.tsx';
 ...
 * 文件名：info.tsx
 */
export interface CategoryInfoExport {
...
// 当前文件: src/books/ComponentsPreview/data/componentsPreviewLoader.tsx
// 目标目录: src/books/ComponentsPreview/**/*.{tsx,md,mdx}
 ...
 * 例如：[1]button.tsx => order: 1, slug: button
...
function parsePath(path: string): {
  ...
  // 使用相对路径匹配
  // 路径格式: ../[01]基础组件/[1]button.tsx
  //          ../[01]基础组件/[2]guide.md
  //          ../[01]基础组件/[3]tutorial.mdx
...
function groupAndSortDocuments(): DocumentCategory[] {
  ...
  Object.entries(documentModules).forEach(([path, module]) => {
    ...
    // 解析文件夹名称
    let categoryInfo: ReturnType<typeof parseFolderName>;
    try {
    ...
    const fullUrl = `${componentsPreviewConfig.slug}/${routePath}`;
...
// 导出分组后的文档数据
export const componentsPreviewCategories = groupAndSortDocuments();

// 导出扁平化的所有文档（用于路由）
export const allComponentsPreviews = componentsPreviewCategories.flatMap(cat => cat.articles);

// 创建 BookLoader 实现
const componentsPreviewLoader: BookLoader = {
  config: {
    title: componentsPreviewConfig.title,
    slug: componentsPreviewConfig.slug,
    icon: () => componentsPreviewConfig.icon,
    description: componentsPreviewConfig.description
  },

  categories: componentsPreviewCategories,

  getAllArticles(): BookArticle[] {
    return allComponentsPreviews;
  },

  getCurrentArticle(path: string): BookArticle | null {
    const normalizedPath = path.toLowerCase().replace(/^\/+|\/+$/g, '');
    return allComponentsPreviews.find(article =>
  ...
  getNavigationData() {
    return {
      categories: componentsPreviewCategories,
      allArticles: allComponentsPreviews
    };
  },
...
// 生成路由的函数
export function generateComponentsPreviewRoutes(): RouteObject[] {
  return [
    {
      path: componentsPreviewConfig.slug,
      element: (
        <BookLayoutConfigProvider basePrefix={`/${componentsPreviewConfig.slug}`}>
          <BookLayout loader={componentsPreviewLoader} />
      ...
      children: [
        ...
        {
          index: true,
          element: allComponentsPreviews.length > 0
            ? (<DebugRedirect to={allComponentsPreviews[0].routePath} />)
            : <div className="p-6 text-center text-muted-foreground">暂无文章</div>,
        },
        // 所有组件页面 - 用 ArticleContent 包装
        ...componentsPreviewCategories.flatMap((category) =>
          category.articles.map((article) => {
            // 子路由路径（相对于 /{slug}）
            const path = article.routePath;
            return {
              path: path,
              element: <ArticleContent loader={componentsPreviewLoader}>{article.content}</ArticleContent>,
            };
          })
        ),
      ],
...
export default componentsPreviewLoader;
